...
%% States of phases (states) were defined as the NT during phases:
...
%% , where $\mathrm{S_F}$ is the state of fixation, $\mathrm{S_E}$ is encoding, $\mathrm{S_M}$ is maintenance, and $\mathrm{S_R}$ is retrieval; $N_{i}$ is the $i$-th NT coordinate, calculated from 50-ms unit activity data.

%% \subsection{Geometric Medians of States}
%% From the middle 1 second (= 20 x 50-ms bins) of each state, the geometric median was calculated:

%% \begin{equation}
%% \mathrm{g_F} = f_{geometric\_median}(\{N_{i} | i = 1, ..., 20\})
%% \end{equation}

%% \begin{equation}
%% \mathrm{g_E} = f_{geometric\_median}(\{N_{i} | i = 31, ..., 50\})
%% \end{equation}

%% \begin{equation}
%% \mathrm{g_M} = f_{geometric\_median}(\{N_{i} | i = 81, ..., 100\})
%% \end{equation}

%% \begin{equation}
%% \mathrm{g_R} = f_{geometric\_median}(\{N_{i} | i = 131, ..., 150\})
%% \end{equation}

%% , where $\mathrm{g_{F}}$, $\mathrm{g_{E}}$, $\mathrm{g_{M}}$, and $\mathrm{g_{R}}$ are geometric medians for fixation, encoding, maintenance, and retrieval phases, respectively; $\mathrm{f_{geometric\_median}}$ is the function to calculate geometric median (using the Python geom\_median package (\url{https://github.com/krishnap25/geom_median?tab=readme-ov-file}). \cite{pillutla:etal:rfa}); and $N_{i}$ is the $i$-th NT coordinate, calculated from 50-ms unit activity data.\\
%% \\
%% It is note worthy that geometric median is calculated by minimizing the sum of distances to the sample points:
...
%% \subsection{Defining Phase States}
%% States of phases were defined as the NT (8 s with 160 x 50-ms bins) during phases:
...
%% where $S_F$ is the state of fixation, $S_E$ is encoding, $S_M$ is maintenance, and $S_R$ is retrieval; $N_{i}$ is the $i^{th}$ NT coordinate.
...
%% It might be noteworthy that geometric median is calculated by minimizing the sum of distances to the sample points:
...
%% \subsection{The Existence of Memory States in NT}
%% Here, we examined whether states of phases are represented in NTs. To achieve this, we utilized two methodologies: machine learning classification and statistical tests on rank-transformed distance data.
%% \\
%% \indent
%% First, a linear support vector machine classified the NT in a supervised manner for labels of the four phases, achieving significant classification results (Table~\ref{tab:SVC}). Moreover, geometric medians were correctly classified into their respective classes (Figure~\ref{fig:SVC}).
%% \\
%% \indent
%% Second, in regard to the encoding and retrieval phases, the distance from g{E} and g{R} were calculated. The statistical test, for ranked data in the pairwise manner, showed significant results, indicating NT durign Encoding and Retrieval are closer to gE and gR, respectively.
%% \\
%% \indent
%% Together, these results provide evidence for the existence of memory states in NT of Putative CA1 regions.
%% /mnt/nas/ylab/ripple-wm-code/data/CA1/clf
\subsection{Existence of Memory States in NT of Putative CA1 Regions}
We examined whether phase states are represented in NTs using two methodologies: machine learning classification and statistical tests on rank-transformed distance data.
\\
\indent
First, a linear support vector machine classified the NT during the four phases in a supervised manner (Table~\ref{tab:SVC}). Balanced accuracy of Fixation vs. Encoding vs. Maintenance vs. Retrieval classification task was XXX while that of Encoding vs. Retrieval task was XXX +/- XXX. These results were significantaly higher than those acuired using a dammy classifier. The classification metrics showed correlations between set size. Additionally, geometric medians were correctly classified as their respective classes (Figure~\ref{fig:SVC}).
\\
\indent
Second, regarding the encoding and retrieval phases, distances of NTs from $g_{E}$ and $g_{R}$ were calculated and converted as rank within each session. Wilcoxon statistical tests revealed that XXX, indicating NT during Encoding and Retrieval are closer to $g_{E}$ and $g_{R}$, respectively (Figure~\ref{fig:dist_wilcoxon}).
\\
\indent
To summarize, these results provide evidence for the existence of memory states in NT of putative CA1 regions.
