...
%DIF ADD ./main/manuscript.tex                    Sat Sep  7 11:07:02 2024
...
\\
\indent
\DIFdelbegin \DIFdel{For each NT within a region such as AHL, geometric medians ($\mathrm{g_{F}}$ for fixation, $\mathrm{g_{E}}$ for encoding, $\mathrm{g_{M}}$ for maintenance, and $\mathrm{g_{R}}$ for retrieval phase) were calculated by determining the median coordinates of the NT during the four phases. }\DIFdelend An optimal GPFA dimensionality was found to be three using the elbow method obtained by examining the log-likelihood values through a three-fold cross-validation approach (Figure~\ref{fig:02}B).

%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF >  \subsection{Calculation of Geometric Medians of NT}
...
%DIF >  \text{The geometric median of } x_1, \dots, x_n \in \mathbb{R}^d \text{ with weights } \alpha_1, \dots, \alpha_n \geq 0 \text{ is defined as}
%DIF >  x_{GM} = \arg\min_{z \in \mathbb{R}^d} \sum_{i=1}^n \alpha_i \|z - x_i\|_2.
\DIFaddbegin 


%DIF > % \subsection{Defining Phase States}
%DIF > % States of phases (states) were defined as the NT during phases:

%DIF > % \begin{equation}
%DIF > % \mathrm{S_F} = \{N_{i} | i = 1, ..., 20\}
%DIF > % \end{equation}

%DIF > % \begin{equation}
%DIF > % \mathrm{S_E} = \{N_{i} | i = 21, ..., 60\}
%DIF > % \end{equation}

%DIF > % \begin{equation}
%DIF > % \mathrm{S_M} = \{N_{i} | i = 61, ..., 120\}
%DIF > % \end{equation}

%DIF > % \begin{equation}
%DIF > % \mathrm{S_R} = \{N_{i} | i = 121, ..., 160\}
%DIF > % \end{equation}


%DIF > % , where $\mathrm{S_F}$ is the state of fixation, $\mathrm{S_E}$ is encoding, $\mathrm{S_M}$ is maintenance, and $\mathrm{S_R}$ is retrieval; $N_{i}$ is the $i$-th NT coordinate, calculated from 50-ms unit activity data.

%DIF > % \subsection{Geometric Medians of States}
%DIF > % From the middle 1 second (= 20 x 50-ms bins) of each state, the geometric median was calculated:

%DIF > % \begin{equation}
%DIF > % \mathrm{g_F} = f_{geometric\_median}(\{N_{i} | i = 1, ..., 20\})
%DIF > % \end{equation}

%DIF > % \begin{equation}
%DIF > % \mathrm{g_E} = f_{geometric\_median}(\{N_{i} | i = 31, ..., 50\})
%DIF > % \end{equation}

%DIF > % \begin{equation}
%DIF > % \mathrm{g_M} = f_{geometric\_median}(\{N_{i} | i = 81, ..., 100\})
%DIF > % \end{equation}

%DIF > % \begin{equation}
%DIF > % \mathrm{g_R} = f_{geometric\_median}(\{N_{i} | i = 131, ..., 150\})
%DIF > % \end{equation}

%DIF > % , where $\mathrm{g_{F}}$, $\mathrm{g_{E}}$, $\mathrm{g_{M}}$, and $\mathrm{g_{R}}$ are geometric medians for fixation, encoding, maintenance, and retrieval phases, respectively; $\mathrm{f_{geometric\_median}}$ is the function to calculate geometric median (using the Python geom\_median package (\url{https://github.com/krishnap25/geom_median?tab=readme-ov-file}). \cite{pillutla:etal:rfa}); and $N_{i}$ is the $i$-th NT coordinate, calculated from 50-ms unit activity data.\\
%DIF > % \\
%DIF > % It is note worthy that geometric median is calculated by minimizing the sum of distances to the sample points:
%DIF > % %% the geometric median of $x_1, \dots, x_n \in \mathbb{R}^d$ is defined as
%DIF > % \begin{equation}
%DIF > % $x_{GM} = \arg\min_{z \in \mathbb{R}^d} \sum_{i=1}^n \|z - x_i\|_2$.
%DIF > % \end{equation}
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%DIF > % \subsection{Defining Phase States}
%DIF > % States of phases were defined as the NT (8 s with 160 x 50-ms bins) during phases:
...
%DIF > % \begin{equation}
%DIF > % S_E = \{N_{i} | i = 21, ..., 60\}
%DIF > % \end{equation}

%DIF > % \begin{equation}
%DIF > % S_M = \{N_{i} | i = 61, ..., 120\}
%DIF > % \end{equation}

%DIF > % \begin{equation}
%DIF > % S_R = \{N_{i} | i = 121, ..., 160\}
%DIF > % \end{equation}


%DIF > % where $S_F$ is the state of fixation, $S_E$ is encoding, $S_M$ is maintenance, and $S_R$ is retrieval; $N_{i}$ is the $i^{th}$ NT coordinate.

%DIF > % \subsection{Geometric Medians of States}
%DIF > % From the middle 1 second (= 20 x 50-ms bins) of each state, the geometric median was calculated:

%DIF > % \begin{equation}
%DIF > % g_F = f_{GM}(\{N_{i} | i = 1, ..., 20\})
%DIF > % \end{equation}

%DIF > % \begin{equation}
%DIF > % g_E = f_{GM}(\{N_{i} | i = 31, ..., 50\})
%DIF > % \end{equation}

%DIF > % \begin{equation}
%DIF > % g_M = f_{GM}(\{N_{i} | i = 81, ..., 100\})
%DIF > % \end{equation}

%DIF > % \begin{equation}
%DIF > % g_R = f_{GM}(\{N_{i} | i = 131, ..., 150\})
%DIF > % \end{equation}

%DIF > % where $g_{F}$, $g_{E}$, $g_{M}$, and $g_{R}$ are geometric medians for fixation, encoding, maintenance, and retrieval phases, respectively; $f_{GM}$ is the function to calculate geometric median (using the Python geom\_median package (\url{https://github.com/krishnap25/geom_median?tab=readme-ov-file}). \cite{pillutla:etal:rfa}); and $N_{i}$ is the $i^{th}$ NT coordinate, calculated from 50-ms unit activity data.

%DIF > % It might be noteworthy that geometric median is calculated by minimizing the sum of distances to the sample points:
...
\DIFadd{where $g_{F}$, $g_{E}$, $g_{M}$, and $g_{R}$ are geometric medians for fixation, encoding, maintenance, and retrieval phases, respectively; $f_{GM}$ is the function to calculate geometric median using the Python geom\_median package (}\url{https://github.com/krishnap25/geom_median?tab=readme-ov-file}\DIFadd{) \mbox{%DIFAUXCMD
...
%DIF > % For each NT within an ROI, the geometric median (or also known the Fermat point, Weber's L1 median, Fréchet median, and so on) of the middle 1 second of each phase was calculated and assigned as $\mathrm{g_{F}}$ for the fixation phase, $\mathrm{g_{E}}$ for the encoding phase, $\mathrm{g_{M}}$ for the maintenance phase, and $\mathrm{g_{R}}$ for the retrieval phase. The calculation of geometric medians was performed using the Python geom\_median package (\url{https://github.com/krishnap25/geom_median?tab=readme-ov-file}). \cite{pillutla:etal:rfa} Specifically,
%DIF > % the geometric median of $x_1, \dots, x_n \in \mathbb{R}^d$ is defined as
%DIF > % $x_{GM} = \arg\min_{z \in \mathbb{R}^d} \sum_{i=1}^n \|z - x_i\|_2$.
%DIF > % \\
\indent
...
\\
\indent
\DIFaddbegin \subsection{\DIFadd{Classification of NT Phases Using Linear SVM Classifier in Putative CA1 Regions}}
\DIFadd{Classification was conducted using a linear support vector machine classifier from the scikit-learn package (\mbox{%DIFAUXCMD
\cite{10.5555/1953048.2078195}}\hspace{0pt}%DIFAUXCMD
) for two conditions: (i) Fixation vs. Encoding vs. Maintenance vs. Retrieval and (ii) Encoding vs. Retrieval phases. Evaluation was performed using 10-fold cross-validation with 100 repetitions. A dummy classifier was employed as a control. Trained weights and biases were utilized to visualize decision boundaries.
}\indent
\\
\indent
\subsection{\DIFadd{NT Distance from Geometric Medians for Phases}}
\DIFadd{Euclidean distances of NT during Encoding/Retrieval phases from the geometric medians of $g_{E}$/$g_{R}$ were calculated. These distances were then transformed into ranks within each session for normalization.
}\indent
\\
\indent
...
%DIF > % \subsection{The Existence of Memory States in NT}
%DIF > % Here, we examined whether states of phases are represented in NTs. To achieve this, we utilized two methodologies: machine learning classification and statistical tests on rank-transformed distance data.
%DIF > % \\
%DIF > % \indent
%DIF > % First, a linear support vector machine classified the NT in a supervised manner for labels of the four phases, achieving significant classification results (Table~\ref{tab:SVC}). Moreover, geometric medians were correctly classified into their respective classes (Figure~\ref{fig:SVC}).
%DIF > % \\
%DIF > % \indent
%DIF > % Second, in regard to the encoding and retrieval phases, the distance from g{E} and g{R} were calculated. The statistical test, for ranked data in the pairwise manner, showed significant results, indicating NT durign Encoding and Retrieval are closer to gE and gR, respectively.
%DIF > % \\
%DIF > % \indent
%DIF > % Together, these results provide evidence for the existence of memory states in NT of Putative CA1 regions.
%DIF > % /mnt/nas/ylab/ripple-wm-code/data/CA1/clf
\DIFaddbegin \subsection{\DIFadd{Existence of Memory States in NT of Putative CA1 Regions}}
\DIFadd{We examined whether phase states are represented in NTs using two methodologies: machine learning classification and statistical tests on rank-transformed distance data.
}\\
\indent
\DIFadd{First, a linear support vector machine classified the NT during the four phases in a supervised manner (Table~\ref{tab:SVC}). Balanced accuracy of Fixation vs. Encoding vs. Maintenance vs. Retrieval classification task was XXX while that of Encoding vs. Retrieval task was XXX +/- XXX. These results were significantaly higher than those acuired using a dammy classifier. The classification metrics showed correlations between set size. Additionally, geometric medians were correctly classified as their respective classes (Figure~\ref{fig:SVC}).
}\\
\indent
\DIFadd{Second, regarding the encoding and retrieval phases, distances of NTs from $g_{E}$ and $g_{R}$ were calculated and converted as rank within each session. Wilcoxon statistical tests revealed that XXX, indicating NT during Encoding and Retrieval are closer to $g_{E}$ and $g_{R}$, respectively (Figure~\ref{fig:dist_wilcoxon}).
}\\
\indent
\DIFadd{To summarize, these results provide evidence for the existence of memory states in NT of putative CA1 regions.
}
...
\end{table*}
\restoregeometry
\DIFaddbegin \pdfbookmark[2]{ID 04}{id_04}
\begin{table*}[htbp]
\centering
\small
\begin{tabular}{*{10}{c}}
...
\midrule
\DIFaddFL{0 }& \DIFaddFL{0.411 +/- 0.111 }& \DIFaddFL{0.246 +/- 0.092 }& \DIFaddFL{-14.313 }& \DIFaddFL{0.0 }& \DIFaddFL{185.417 }& \DIFaddFL{0.137 }& \DIFaddFL{01 }& \DIFaddFL{01 }& \DIFaddFL{AHL}\\
\rowcolor{lightgray}
\DIFaddFL{1 }& \DIFaddFL{0.383 +/- 0.109 }& \DIFaddFL{0.253 +/- 0.099 }& \DIFaddFL{-10.034 }& \DIFaddFL{0.0 }& \DIFaddFL{197.633 }& \DIFaddFL{0.197 }& \DIFaddFL{01 }& \DIFaddFL{02 }& \DIFaddFL{AHL}\\
\DIFaddFL{2 }& \DIFaddFL{0.381 +/- 0.101 }& \DIFaddFL{0.232 +/- 0.091 }& \DIFaddFL{-13.838 }& \DIFaddFL{0.0 }& \DIFaddFL{181.167 }& \DIFaddFL{0.143 }& \DIFaddFL{03 }& \DIFaddFL{01 }& \DIFaddFL{AHR}\\
\rowcolor{lightgray}
\DIFaddFL{3 }& \DIFaddFL{0.428 +/- 0.106 }& \DIFaddFL{0.244 +/- 0.103 }& \DIFaddFL{-17.52 }& \DIFaddFL{0.0 }& \DIFaddFL{192.872 }& \DIFaddFL{0.109 }& \DIFaddFL{03 }& \DIFaddFL{02 }& \DIFaddFL{AHR}\\
\DIFaddFL{4 }& \DIFaddFL{0.407 +/- 0.093 }& \DIFaddFL{0.264 +/- 0.104 }& \DIFaddFL{-12.735 }& \DIFaddFL{0.0 }& \DIFaddFL{178.127 }& \DIFaddFL{0.152 }& \DIFaddFL{04 }& \DIFaddFL{01 }& \DIFaddFL{PHL}\\
\rowcolor{lightgray}
\DIFaddFL{5 }& \DIFaddFL{0.486 +/- 0.105 }& \DIFaddFL{0.252 +/- 0.091 }& \DIFaddFL{-32.141 }& \DIFaddFL{0.0 }& \DIFaddFL{197.049 }& \DIFaddFL{0.053 }& \DIFaddFL{04 }& \DIFaddFL{02 }& \DIFaddFL{PHL}\\
\DIFaddFL{6 }& \DIFaddFL{0.346 +/- 0.090 }& \DIFaddFL{0.259 +/- 0.095 }& \DIFaddFL{-7.152 }& \DIFaddFL{0.0 }& \DIFaddFL{197.016 }& \DIFaddFL{0.257 }& \DIFaddFL{06 }& \DIFaddFL{01 }& \DIFaddFL{AHL}\\
\rowcolor{lightgray}
\DIFaddFL{7 }& \DIFaddFL{0.329 +/- 0.086 }& \DIFaddFL{0.239 +/- 0.090 }& \DIFaddFL{-7.963 }& \DIFaddFL{0.0 }& \DIFaddFL{194.355 }& \DIFaddFL{0.238 }& \DIFaddFL{06 }& \DIFaddFL{02 }& \DIFaddFL{AHL}\\
\DIFaddFL{8 }& \DIFaddFL{0.507 +/- 0.108 }& \DIFaddFL{0.254 +/- 0.100 }& \DIFaddFL{-38.482 }& \DIFaddFL{0.0 }& \DIFaddFL{197.153 }& \DIFaddFL{0.041 }& \DIFaddFL{09 }& \DIFaddFL{01 }& \DIFaddFL{AHR}\\
\rowcolor{lightgray}
\DIFaddFL{9 }& \DIFaddFL{0.490 +/- 0.098 }& \DIFaddFL{0.265 +/- 0.091 }& \DIFaddFL{-34.298 }& \DIFaddFL{0.0 }& \DIFaddFL{186.034 }& \DIFaddFL{0.046 }& \DIFaddFL{09 }& \DIFaddFL{02 }& \DIFaddFL{AHR}\\
\DIFaddFL{10 }& \DIFaddFL{0.417 +/- 0.117 }& \DIFaddFL{0.251 +/- 0.096}\\
\bottomrule
\end{tabular}
\captionsetup{width=\textwidth}

\caption{\textbf{
\DIFaddFL{SVC Classification of States
}}
\smallskip
\\
\DIFaddFL{SVC classification of NT states revealed linear distinguishability, confirming the existence of distinct states in NT spaces.
}}
...
%DIFDELCMD <         	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\textwidth]{./src/figures/png/Figure_ID_03.png}
        	\DIFaddendFL \caption{\textbf{
...
%DIFDELCMD <         	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\textwidth]{./src/figures/png/Figure_ID_05.png}
...
%DIFDELCMD <         	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.5\textwidth]{./src/figures/png/Figure_ID_07.png}
        	\DIFaddendFL \caption{\textbf{
