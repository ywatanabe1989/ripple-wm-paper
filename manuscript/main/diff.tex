...
%DIF ADD ./main/manuscript.tex                    Sun Aug 18 09:03:27 2024
...
\\
\indent
\DIFdelbegin \DIFdel{For each NT within a region such as AHL, geometric medians ($\mathrm{g_{F}}$ }\DIFdelend \DIFaddbegin \DIFadd{An optimal GPFA dimensionality was found to be three using the elbow method obtained by examining the log-likelihood values through a three-fold cross-validation approach (Figure~\ref{fig:02}B).
}
...
%DIF > % \\
\indent
%DIF > % \subsection{Topological Data Analysis}
%DIF > % Geometric medians were considered as the centroid of each class of samples. From these centroids, radii were incrementally increased, and the number of "holes" was counted in response to the radius. This process generates barcode plots to evaluate the stability of each state and persistent homology.
%DIF > % \\
\indent
\subsection{\DIFadd{Classification of NT Phases Using a Linear SVM Classifier}}
\DIFadd{A linear support vector machine classifier was applied to NT data from each session to classify }\DIFaddend the four phases \DIFdelbegin \DIFdel{. An optimal GPFA dimensionality was found to be three using the elbow method obtained by examining the log-likelihood values through a }\DIFdelend \DIFaddbegin \DIFadd{of the task: Fixation, Encoding, Maintenance, and Retrieval, in a supervised manner. Evaluation was conducted using the }\DIFaddend three-fold cross-validation \DIFdelbegin \DIFdel{approach (Figure~\ref{fig:02}B).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{method. The classification tasks were also conducted under specific conditions---set size (4, 6, or 8) and task type (Match IN and Mismatch OUT)---as well as an entire session with 50 trials.
}\\
\indent
...
\DIFaddbegin \subsection{\DIFadd{The Existance of Memory States in NT}}
\DIFadd{Here, we examined whether NT representations linearly depict memory states across the four phases. To achieve this, we utilized two methodologies: topological data analysis (TDA) and machine learning classification.
}\\
\indent
\DIFadd{First, geometric medians were treated as centroids for phases and the number of holes were calculated using the TDA.
}\\
\indent
\DIFadd{Second,
}\\
\indent
\DIFadd{Together, these results provide compelling evidence for the existence of memory states in NT, as revealed by both TDA and ML classification.
}
