...
\subsection{Defining Phase States}
States of phases were defined as the NT during phases:
...
where $S_F$ is the state of fixation, $S_E$ is encoding, $S_M$ is maintenance, and $S_R$ is retrieval; $N_{i}$ is the $i^{th}$ NT coordinate, calculated from 50-ms unit activity data.
...
It is noteworthy that geometric median is calculated by minimizing the sum of distances to the sample points:
%% the geometric median of $x_1, \dots, x_n \in \mathbb{R}^d$ is defined as
\begin{equation}
x_{GM} = \arg\min_{z \in \mathbb{R}^d} \sum_{i=1}^n \|z - x_i\|_2.
\end{equation}
...
\subsection{The Existance of Memory States in NT}
Here, we examined whether NT representations linearly depict memory states across the four phases. To achieve this, we utilized two methodologies: topological data analysis (TDA) and machine learning classification.
\\
\indent
First, geometric medians were treated as centroids for phases and the number of holes were calculated using the TDA.
\\
\indent
Second,
\\
\indent
Together, these results provide compelling evidence for the existence of memory states in NT, as revealed by both TDA and ML classification.
