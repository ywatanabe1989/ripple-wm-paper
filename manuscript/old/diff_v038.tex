...
%DIF ADD ./manuscript.tex                           Sun May  5 19:07:28 2024
...
Having observed NT 'jumping' during SWR (Figure~\ref{fig:05}), we visualized the three-dimensional NTs of pre-, mid-, and post-SWR events during the encoding and retrieval phases (Figure~\ref{fig:06})\DIFdelbegin \DIFdel{. The distance between these was found to be }\DIFdelend \DIFaddbegin \DIFadd{, the combination of which showed }\DIFaddend memory-load dependent \DIFaddbegin \DIFadd{NT distances }\DIFaddend (Figure~\ref{fig:03}). 
\\
\indent
...
}\DIFaddend . However, in the present study, applying GPFA to multi-unit activity during a one-second level resolution of the WM task revealed that the neural NT in low-dimensional space presented a memory-load dependency between the encoding and retrieval phases, denoted as $\mathrm{\lVert g_{E}g_{R} \rVert}$ (Figure~\ref{fig:03}). These findings support the association of the hippocampus with WM processing.
